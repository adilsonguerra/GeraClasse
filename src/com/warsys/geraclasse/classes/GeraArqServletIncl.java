...
public class GeraArqServletIncl {
	
	boolean geracao_ok = false;
	
	public GeraArqServletIncl(String nomeClasse,String nomeEmpSist, Registros[] regConvertido){
			...
			try{
				...
				a.escrever("  ",0);
				a.escrever("public class "+GeraClasse.nomeServletInclusao+" extends HttpServlet {",0);
				a.escrever("",0);
				...
				a.escrever("                          throws ServletException, IOException {",0);
				a.escrever("",0);
				a.escrever("  System.out.println("+'\"'+"Entrou no "+GeraClasse.nomeServletInclusao+"..."+'\"'+");",0);
				a.escrever("",0);				
								    ...
								    a.escrever("      "+regConvertido[i].getColuna()+"= Float.parseFloat(request.getParameter("+'\"'+ regConvertido[i].getColuna()+'\"'+").replace(',','.'));",0);						  
									      ...
									      a.escrever("      "+regConvertido[i].getColuna()+"= Double.parseDouble(request.getParameter("+'\"'+ regConvertido[i].getColuna()+'\"'+").replace(',','.'));",0);
									           ...
									           a.escrever("      "+regConvertido[i].getColuna()+"= Short.parseShort(request.getParameter("+'\"'+ regConvertido[i].getColuna()+'\"'+"));",0);
								 					    ...
								 					    a.escrever("      "+regConvertido[i].getColuna()+"= Long.parseLong(request.getParameter("+'\"'+ regConvertido[i].getColuna()+'\"'+"));",0);
										          }else{if(regConvertido[i].getTipo().equals("java.math.BigDecimal")){
								 					         a.escrever("      "+regConvertido[i].getColuna()+"= new java.math.BigDecimal(request.getParameter("+'\"'+ regConvertido[i].getColuna()+'\"'+").replace(',','.'));",0);
               ...
               a.escrever("      out.println(MontaPgErro(\"Ocorreu um erro de conversão! Verifique se dado selecionado está correto.\",e.getMessage()));",0);              
					...
					a.escrever("   	 System.out.println(" +'\"'+"Ocorreu um erro ao INCLUIR o registro!" +'\"'+"+  e.getMessage());",0);
					a.escrever("",0);
					a.escrever("   	 // acerta tipo MIME para a resposta",0);
					a.escrever("   	 response.setContentType("+'\"'+"text/html"+'\"'+");",0);
					a.escrever("",0);
               ...
               a.escrever("      out.println(MontaPgErro(\"Ocorreu um erro ao INCLUIR! Verifique se dado selecionado está correto.\",e.getMessage()));",0);              

					a.escrever("	   out.close();",0);
					a.escrever("",0);
					a.escrever("   RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("+'\"'+"/Grid_Campos"+GeraClasse.nomeBean+".jsp" +'\"'+");",0);
					a.escrever("   dispatcher.forward(request,response);",0);
					a.escrever("",0);
					a.escrever("   }",0);
				...
				a.escrever("",0);
				a.escrever("   System.out.println("+'\"'+"Estou no "+GeraClasse.nomeServletExclusao+'\"'+");",0);
				a.escrever("",0);
            ...
            a.escrever("      out.println(MontaPgErro(\"Ocorreu um erro ao INCLUIR! Verifique se dado selecionado está correto.\",e.getMessage()));",0);              
				...
				a.escrever("   dispatcher.forward(request,response);",0);
				a.escrever("",0);
				a.escrever("System.out.println("+'\"'+"...saiu do "+GeraClasse.nomeServletInclusao+'\"'+");",0);
				a.escrever("",0);				
